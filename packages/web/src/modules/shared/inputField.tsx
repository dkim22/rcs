...
> = ({ field: { onChange, ...field }, form: { errors, setFieldValue }, label, useNumberComponent = false, ...props }) => {
  ...
  return (
    <Form.Item label={label} help={errorMsg} validateStatus={errorMsg ? "error" : undefined}>
      <Comp {...field} {...props} onChange={useNumberComponent ? (newValue: string) => setFieldValue(field.name, newValue) : onChange} />
    </Form.Item>
  );
};
